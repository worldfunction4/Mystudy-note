OSPF（Open Shortest Path First， **开放式路径最短优先**）是一种基于链路状态算法的**内部网关协议（[[IGP协议]]）**，专为大型网络设计。它收敛快、无环路、支持VLSM/CIDR，是目前企业网和数据中心最核心的路由协议。
...
为了更好理解OSPF的[[OSPF#^b4473b|七种状态]]，我在eNSP上进行了抓包实验。
